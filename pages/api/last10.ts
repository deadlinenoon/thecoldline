...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  try{
    ...
    for (const season of seasons){
      ...
      for (const ev of events) {
        ...
        const opponent = String(opp?.team?.displayName || opp?.team?.name || '');
        const result: 'W'|'L'|'T' = pf>pa? 'W' : (pa>pf? 'L':'T');
        games.push({ date: when.toISOString(), opponent, homeAway, pf, pa, result });
      }
  ...
  }catch(e:any){ return res.status(500).json({ error:e?.message||'last10 error' }); }
}
